...
const handler = async (event: { body: string, httpMethod: string }) => {
    ...
    try {
        ...
        switch (action) {
            case 'analyze':
                return analyze(ai, model, payload);
            case 'rewrite':
                return rewrite(ai, model, payload);
            case 'find_jobs':
                return findJobs(ai, model, payload);
            case 'generate_pitch':
                return generatePitch(ai, model, payload);
            case 'generate_linkedin':
                return generateLinkedIn(ai, model, payload);
            default:
        ...
        return { statusCode: 500, body: JSON.stringify({ error: `Internal Server Error: ${errorMsg}` }) };
    }
};

// --- Action Handlers ---

async function analyze(ai: GoogleGenAI, model: string, payload: { jobDescription: string; resume: string; }) {
    const { jobDescription, resume } = payload;
    const prompt = `
...
`;
    ...
    const parsedResult: AnalysisResult = JSON.parse(response.text.trim());
    return { statusCode: 200, body: JSON.stringify(parsedResult) };
}

async function rewrite(ai: GoogleGenAI, model: string, payload: { originalResume: string; jobDescription: string; modernizationSuggestions: ModernizationSuggestion[] }) {
    const { originalResume, jobDescription, modernizationSuggestions } = payload;
...
`;
    ...
    return { statusCode: 200, body: JSON.stringify({ rewrittenResume: response.text.trim() }) };
}

async function findJobs(ai: GoogleGenAI, model: string, payload: { resume: string }) {
...
`;
    ...
    const parsedResult: JobSuggestion[] = JSON.parse(response.text.trim());
    return { statusCode: 200, body: JSON.stringify(parsedResult) };
}

async function generatePitch(ai: GoogleGenAI, model: string, payload: { resume: string; jobDescription: string }) {
...
`;
    const response = await ai.models.generateContent({ model, contents: prompt });
    return { statusCode: 200, body: JSON.stringify({ pitch: response.text.trim() }) };
}

async function generateLinkedIn(ai: GoogleGenAI, model: string, payload: { resume: string; jobDescription: string }) {
    const { resume, jobDescription } = payload;
    const prompt = `
...
`;
    ...
    const parsedResult: LinkedInGeneratorResult = JSON.parse(response.text.trim());
    return { statusCode: 200, body: JSON.stringify(parsedResult) };
}
